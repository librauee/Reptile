# -*- coding: utf-8 -*-
"""
Created on Wed Feb 6 15:34:34 2019
...
class CommentPhotoCrawler(object):
    ...
    def download_photo(self, output='.'):
        ...
        """
        os.mkdir(output+'/photos2')
        with open('photourl1.txt', 'r') as f:
            photo_urls = [url.strip() for url in f.readlines()]
        for url in photo_urls:
            res = requests.get(url)
            name = url.split('/')[-1]  # 取出图片名
            with open(output+'/photos2/'+name, 'wb') as f:
                f.write(res.content)
...
if __name__ == '__main__':
    ...
    com.get_m_url('https://weibo.com/1713926427/HhqnnaMpK')
    com.login('username', 'password')  # 传入你的微博用户名和密码
